...
Java_com_example_untrunc_MainActivity_stringFromJNI(JNIEnv *env, jobject thiz,
													jstring ok,
													jstring corrupt) {
	try {
		Mp4 mp4;
...
//		auto ext = getMovExtension(jstring2str(env, ok));
//		mp4.makeStreamable(jstring2str(env, ok), jstring2str(env, ok) + "_streamable" + ext);
//		if (mp4.alreadyRepaired(jstring2str(env, ok), jstring2str(env, corrupt))) return 0;

		LOGD("reading... ", "%s", Jstring2CStr0(env, ok));
		logg(I, "reading ", ok, '\n');
		mp4.parseOk(jstring2str(env, ok), false);
		mp4.repair(jstring2str(env, corrupt));
	}
	catch (const char* e) {
		LOGD("catch. ", "%s", e);
		return cerr << e << '\n', 1;}
	catch (string e) {
		printf("catch %s", e.c_str());
		LOGD("catch.. ", "%s", e.c_str());
		return cerr << e << '\n', 1;}
//	"Could not create file '/sdcard/byteflow/learnffmpeg/av_20220508203656.mp4_fixed.mp4': Operation not permitted"
	return 0;
}